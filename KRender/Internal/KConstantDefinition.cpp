	...
	void SafeInit()
	{
		...
		if(!CONSTANT_DETAIL_INIT)
		{
			...
			// CASCADED_SHADOW_DETAILS
			{
				// LIGHT_VIEW
				{
					ConstantSemanticDetail DETAIL = { CS_CASCADED_SHADOW_VIEW, EF_R32G32B32A32_FLOAT, 4, MEMBER_SIZE(CASCADED_SHADOW, LIGHT_VIEW), MEMBER_OFFSET(CASCADED_SHADOW, LIGHT_VIEW) };
					CASCADED_SHADOW_DETAILS.semanticDetails.push_back(DETAIL);
				}
				// LIGHT_PROJ
				{
					ConstantSemanticDetail DETAIL = { CS_CASCADED_SHADOW_PROJ, EF_R32G32B32A32_FLOAT, 4 * 4, MEMBER_SIZE(CASCADED_SHADOW, LIGHT_PROJ), MEMBER_OFFSET(CASCADED_SHADOW, LIGHT_PROJ) };
					CASCADED_SHADOW_DETAILS.semanticDetails.push_back(DETAIL);
				}
				// FRUSTRUM
				{
					ConstantSemanticDetail DETAIL = { CS_CASCADED_SHADOW_FRUSTRUM, EF_R32_FLOAT, 4, MEMBER_SIZE(CASCADED_SHADOW, FRUSTRUM), MEMBER_OFFSET(CASCADED_SHADOW, FRUSTRUM) };
					CASCADED_SHADOW_DETAILS.semanticDetails.push_back(DETAIL);
				}
				// CAM_NEAR_FAR
				{
					ConstantSemanticDetail DETAIL = { CS_CASCADED_SHADOW_NEAR_FAR, EF_R32G32_FLOAT, 4, MEMBER_SIZE(CASCADED_SHADOW, CAM_NEAR_FAR), MEMBER_OFFSET(CASCADED_SHADOW, CAM_NEAR_FAR) };
					CASCADED_SHADOW_DETAILS.semanticDetails.push_back(DETAIL);
				}
