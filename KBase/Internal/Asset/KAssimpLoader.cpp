...
KAssimpLoader::KAssimpLoader()
{
	KAssimpIOHooker* hooker = KNEW KAssimpIOHooker(KFileSystem::Manager.get());
...
bool KAssimpLoader::ImportAiScene(const aiScene* scene, const KAssetImportOption& importOption, KMeshRawData& result)
{
	...
	for (unsigned int i = 0; i < scene->mNumMeshes; i++)
	{
		...
		{
			KFileTool::PathJoin(m_AssetFolder, diffuseMap.C_Str(), part.material.urls[MTS_DIFFUSE]);
		}
		...
		{
			KFileTool::PathJoin(m_AssetFolder, emissiveMap.C_Str(), part.material.urls[MTS_EMISSIVE]);
		}

		aiString aoMap;
		if (aiReturn_SUCCESS == scene->mMaterials[paiMesh->mMaterialIndex]->GetTexture(aiTextureType_AMBIENT, 0, &aoMap))
		{
			KFileTool::PathJoin(m_AssetFolder, aoMap.C_Str(), part.material.urls[MTS_AMBIENT_OCCLUSION]);
		}
		...
		{
			KFileTool::PathJoin(m_AssetFolder, specaulrMap.C_Str(), part.material.urls[MTS_SPECULAR_GLOSINESS]);
		}
		...
		{
			KFileTool::PathJoin(m_AssetFolder, normalMap.C_Str(), part.material.urls[MTS_NORMAL]);
		}
